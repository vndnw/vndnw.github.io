const slider = (function(){
	...
	function handleActiveNavbar (index) {
		// Navbar Laptop
	...
	function _createControls(dots = false, buttons = false) {
		...
		const arrayMenu = [['homeId', 'aboutId', 'resumeId'], ['homeMobileId', 'aboutMobileId', 'resumeMobileId']];

		for (let i = 0; i < arrayMenu.length; i ++) {
			for (let j = 0; j < arrayMenu[i].length; j++) {
				document.getElementById(arrayMenu[i][j]).addEventListener("click", () => {
					updateItemsInfo(j);
					handleActiveNavbar(j);
				});
			}
		}
